...
try:
    ...
    st.error("Required package 'joblib' is not installed.\nInstall it with `pip install joblib` or `pip install -r requirements.txt` and restart the app.")
    st.stop()
...
# ══════════════════════════════════════════════════════════
    ...
    with tab2:
        if not artifacts_ready or not data_ready:
            ...
            @st.cache_data
            def _get_split(data_path_str: str):
                return load_and_preprocess(Path(data_path_str))

            (X_tr, X_te, y_tr, y_te,
             fn, pp, _) = _get_split(str(ROOT / "data" / "employees.csv"))
